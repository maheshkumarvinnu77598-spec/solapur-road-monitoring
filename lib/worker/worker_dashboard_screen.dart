...
class _WorkerDashboardScreenState extends State<WorkerDashboardScreen> {
      ...
      builder:
          (
            ...
            final int slaWarnings = reports
                .where((r) => !completed && _isNearSlaBreach(r))
                .length;
  ...
  Future<void> _uploadRepair(String reportId) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Repair proof uploaded successfully')),
      );
    } catch (_) {
      if (!mounted) {
        return;
    ...
    final String? selected = await showDialog<String>(
      ...
      builder: (BuildContext context) {
        return SimpleDialog(
          ...
          children: [
            for (final status in const ['Assigned', 'In Progress'])
              SimpleDialogOption(
  ...
  String _slaLabel(ReportModel report) {
    if (report.assignedAt == null || report.status == 'Resolved') {
      return 'SLA On Track';
    }
